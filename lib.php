...
{    
    ...
    public static function faNormalize($string, $normalizeDigits = false, $htmlNormalize = false)
    {
    ...
    public static function ye($string, $normalizeDigits = false)
    {
        if (gettype($string) == 'array' or gettype($string) == 'object') {
            foreach ($string as $key => $val) {
    ...
    public static function faNormalizeNumber($val, $normalizeDigits = true, $htmlNormDigit = false)
    {
        return self::faNormalize(number_format($val, 0, ".", ","), $normalizeDigits, $htmlNormDigit);
    }

    public static function faNormalizejDate($val)
    {
        return self::faNormalize(jDate("Y/m/d", 0, 0, strtotime($val)), true);
    }

    public static function farsiSort($ar, $order = 'asc', $flags = SORT_STRING)
    {
        foreach ($ar as $k => $v)
            $ar[$k] = faEncode($v);
    ...
    public static function arrayQsort(&$array, $column, $order = 'SORT_ASC', $first = 0, $last = -2)
    {
    ...
    public static function farsiDbSort(&$records, $sortby, $order)
    {
    ...
    public static function faDecode($str)
    {
        $_to_farsi = array(
    ...
    public static function faEncode($str)
    {
        $_to_safe = array(
...
//==============================================convert farsi===========
    ...
    public static function avg2Text($inp)
    {
    ...
    public static function num2Text($input)
    {
    ...
    public static function farsiMail($string)
    {
        $trans = array(
            "0" => "&#1776;", "1" => "&#1777;", "2" => "&#1778;", "3" => "&#1779;", "4" => "&#1780;",
            "5" => "&#1781;", "6" => "&#1782;", "7" => "&#1783;", "8" => "&#1784;", "9" => "&#1785;",
    ...
    public static function utf8SafeSubstr($string, $length, $start = 0)
    {
    ...
    public static function fa2EnDigit($txt)
    {
        $trans = array(
            "۰" => "0", "٠" => "0", "۱" => "1", '١' => '1', "۲" => "2", "٢" => "2", "۳" => "3", "٣" => "3", "۴" => "4",
            "۵" => "5", "۶" => "6", "۷" => "7", "۸" => "8", "۹" => "9", "٩" => "9",
    ...
    public static function en2FaDigit($txt)
    {
        $trans = array(
            "0" => "۰", "1" => "۱", "2" => "۲", "3" => "۳", "4" => "۴",
            "5" => "۵", "6" => "۶", "7" => "۷", "8" => "۸", "9" => "۹");
    ...
    public static function numPad($inp)
    {
        if (strstr($inp, '.')) {
            $d = explode('.', $inp);
            $d[1] = substr($d[1], 0, 2);
...
//  return sprintf("%01.2f", floatval($inp + 0.0000001));
    ...
    public static function autoDirection($str)
    {
    ...
    public static function jMkTime($hour, $minute, $second, $jmonth, $jday, $jyear)
    {
    ...
    public static function convertNum2Farsi($srting) ////here convert to  number in persian
    {
        $num0 = "۰";
        $num1 = "۱";
        $num2 = "۲";
    ...
    public static function gregorian2Jalai($g_y, $g_m, $g_d)
    {
    ...
    public static function jalali2Gregorian($j_y, $j_m, $j_d)
    {
        $g_days_in_month = array(31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31);
        $j_days_in_month = array(31, 31, 31, 31, 31, 31, 30, 30, 30, 30, 30, 29);
...
//############################################################
    ...
    public static function g2jDate($input, $normalizeNumber = false, $seprator = '/')
    {
        ...
        $format = "Y" . $seprator . "m" . $seprator ."d" . (($date['hour'] || $date['minute']) ? " H:i" : "");
...
function farsiDate($param, $format = 'Y/m/d', $farsiDigit = true) {
    $farsiDigit = is_bool($format) ? $format : $farsiDigit;
    $format = !is_bool($format) ? $format : 'Y/m/d';
    $date = lib::jDate($format, 0, 0, strtotime($param));

    return ($farsiDigit ? lib::en2faDigit($date) : $date);
}